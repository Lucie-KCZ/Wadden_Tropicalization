...
#
# Description:
# This script defines a set of functions to process biodiversity data
# and calculate the experienced temperatures per species (CTI).
# It merges temperature data with species occurrence data and
# provides options to plot the results and assess underlying processes 
# (tropicalisation, borealisation, etc.).
#
...
plot_trend <- function(CTI) {
  ...
  pcol <- ifelse(summary(model)$tTable[2,5] > 0.05, 'slategray1', 'violetred1')
  lcol <- ifelse(summary(model)$tTable[2,5] > 0.05, 'slategray3', 'violetred4')
...
get_processes <- function(community_matrix, STI, CTI, infos_col, all_outputs = TRUE, log = TRUE) {
  ...
  if(any(!c('Station', 'Year') %in% colnames(community_matrix))) {
    stop('Columns "Station" or "Year" missing in community_matrix.')
  }
  if(!"CTI" %in% colnames(CTI)) {
    stop('No "CTI" column in CTI data frame.')
  }
...
plot_species <- function(processes) {
  ...
  if (!requireNamespace("scales", quietly = TRUE)) {
    warning("Package 'scales' is required for alpha transparency. Install if needed.")
  }
